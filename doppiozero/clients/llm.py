...
class LLMClient:
    ...
    def _call_openai_api(
        ...
        if self.verbose:
            logger.info("-------------------------------------")
            logger.info("Submitting the request to the LLM (%s)...", request_type)
            logger.info("-------------------------------------")
        ...
        # default: chat/generation
        resp = client.chat.completions.create(
            messages=[{"role": "user", "content": prompt}],
            max_completion_tokens=(
                int(os.environ.get("MAX_TOKENS", 1024)) if max_tokens is None else int(max_tokens)
            ),
        ...
        if self.api_key:
            try:
                response_dc = self._call_openai_api(
                    prompt, request_type="chat", max_tokens=max_tokens
                )
    ...
    def embed(self, text: str, model: Optional[str] = None) -> List[float]:
        ...
        if self.api_key:
            ...
            try:
                ...
                resp = self._call_openai_api(text, request_type="embed", model=model_name)
